...
リンクをクリックしてブログの 'admin' ページにログインすると、ポスト、タグ、コメント、およびユーザのリストにアクセスすることができます。これらのページは "CRUD":http://www.playframework.org/documentation/1.0.1/crud によって提供されるものです。これらそれぞれのページにおいて、その (薄いピンク色の) タイトルと列ヘッダは、アプリケーションのモデル、すなわち JavaBean のクラス名と属性名から来る文字列です。
