...
p(note). Note that we use a **TreeSet** here in order to keep the tag list in a predictible order (alphabetical order in fact, because of our previous compareTo implementation).
...
p(note). Add the Tags declaration at the top of the YAML file, because they need to be created before that any Post reference them.
