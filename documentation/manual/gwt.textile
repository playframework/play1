h1(red). **Google Web Toolkit** support


This module provides helper to simplify the integration of a GWT UI with Play as application server.

h2. <a>First, download the GWT SDK</a>

p. This module is designed to __work with___ the latest version of GWT. A the moment, it is the 1.6.4 version. Before working with the Play GWT module, you have to download the GWT SDK and set the GWT_PATH environment variable.
...
pThen edit the conf/application.conf file to enable the GWTmodule :
...
bq. At the first run, GWT will compile your main module a first time. Now you can make change in your application and refresh the hosted mode browser to see the result.

!images/gwt!

h2. <a>Creating service using GWT-RPC</a>
...
* The first one, run your Play application
* The second one, run the client UI

So if you want to debug all the application you have to attach 2 debugging sessions using JPDA. This is not a problem ...

For example, using netbeans. Start to create a netbeans project with the **play netbeansify** command :

bc. # play netbeansify test-gwt

Now open the **test-gwt** project in netbeans and use the debug button to attach the 2 debugging session. By default the Play application should listen for debugger on port 8000, and the GWT browser on port 3408. 
