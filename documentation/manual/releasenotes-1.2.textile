...
In Play 1.2 we introduce Promise that is the Play's custom Future type. In fact a @Promise<T>@ is also a @Future<T>@ so you can use it as a standard Future. But it has also a very interesting property: the ability to register callback using @onRedeem(…)@ that will be called as soon as the promised value is available. It allows the framework to register itself on them and to reschedule the request invocation as soon as possible.
...
Like the old **staticDir** mapping, you can now directly map an URL path to a static file rendering.
...
For example, the **JPA Plugin** automatically open a database transaction for each invocation if a database is configured. Now if you don't need any database connection for a particular invocation, you can annotate it with **@NoTransaction**:
...
We are now using "H2 Database":http://www.h2database.com/ as in-memory database for Play. It is more compatible with production database like MySQL, so you will have less problem during deployment.
...
You can now load your fixtures from several yaml files in one time, and even use the template engine markup language in your yaml definitions to add some kind of dynamism.
