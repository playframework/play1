...
To create high performance systems, sometimes you need to cache data. Play has a cache library and will use "Memcahed":http://www.danga.com/memcached/ when used in a distributed environment.
...
This method is blocking and returns a boolean value indicating whether he object has been deleted or not. So the full pattern that ensures an item is deleted from the cache is:
...
Note that those being blocking call, **safe*** methods will slow down your application. So use them only when needed.
