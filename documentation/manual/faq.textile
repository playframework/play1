...
Yes play itself is fast. But that does not mean that any particular application will be fast. The embedded HTTP server and the basic routing and controller invocation stack is very very fast. Used with a modern JVM and Just In Time compiling you can get thousand of requests per second easily. Unafortunely as your application will likely use a database it will become the bottleneck (as always).
