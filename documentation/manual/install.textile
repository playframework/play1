...
p(note). If you are using MacOS, java is just built-in. If you are using linux, make sure to use either the Sun-JDK or OpenJDK (and not gcj which is the default java command on many linux distro). If you are using Windows, just download and install the latest JDK package.

Be sure to have java in the current path (try to type **java -version** to check). Play will use the default java or the one available at the **$JAVA_HOME** path if defined.

The play command line utility uses python. So it should work out of the box on any UNIX system (however it requires at least python 2.4). If you're running Windows, don't worry, a python runtime is bundled with the framework.


h2. <a>Download the binary package</a>

Download the latest "play binary package":http://download.playframework.org/ and extract the archive. For convenience, you should add the framework installation directory to your system PATH. If you're on UNIX, make sure that the _play_ script is runnable (otherwise simply do a **chmod +x play**). That's all.
