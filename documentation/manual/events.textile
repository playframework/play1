...
The API to send event is pretty straightforward: an event is a `string` and an `Object`.
...
At the moment the following events are sent by Play framework. The events go in pair signaling the beginning and the end of a framework operation. 

Usually the `context` provided before and after events are the same except for JPQL category. In this case, the before event gives the entity name while the after give the JPQL instance.
