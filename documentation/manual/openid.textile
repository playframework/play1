...
"OpenID":http://openid.net/|OpenID is an open and decentralized identity system. You can easily accept new users in your application without the need to keep specific user information. You just have to keep track of authorized users through their **OpenID**.
...
public static void authenticate(String user) {
    if(user != null) {
        OpenID.verify(user); // will redirect the user
    } else {
        String verifiedUser = OpenID.getVerifiedID();
