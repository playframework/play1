...
This route will invariably accept:
...
The URI pattern defines the request path of the route. Some parts of the request path can be dynamic. Any dynamic part must be specified within braces {…}.
...
will match the */clients* URL but not */clients/*. You can tell Play that you want to match both URLs adding a question mark after the trailing slash. For example:
