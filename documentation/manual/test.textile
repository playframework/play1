...
After the run, all results are saved to the **/test-result** directory. Moreover, this directory contains a marker file (either **result.failed** or **result.passed**) for the final result of the test suite. Finally, this directory contains all the logs, in an **application.log** file.
