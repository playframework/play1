...
bc. #{selenium 'Test email sending'}
	...
	// Extract the last email sent to boron@localhost into a javascript variable called email
...
bc. Fixtures.load("users.yml", "roles.yml", "permissions.yml");
...
h2. <a name="running_outside">Running tests outside the web based runner</a>

Out of the box, tests written for Play cannot be run with your favorite IDE. This is because Play context needs to be loaded in order for tests to succeed. To overcome this limitation, Play test framework provides you with a JUnit4 Rule that initializes Play before running tests. Tests annotated this way can be run **both** inside web based runner **and** inside an IDE.
