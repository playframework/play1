...
Sometimes when dealing with http requests you want to schedule a Job to run after request is responded. A natural use case would be if you create and save new JPA entities and 
the Job has to retrieve them from database by some identifier and process them. Triggering a Job with @now()@ won't work, because transaction is not yet committed, so a thread executing the task 
won't be able to retrieve entities; @in(delay)@ will work, but it is not flexible enough. For those cases @Job@ offers @afterRequest()@ method that will schedule a job to run when request is served.
