...
h2. <a>1. Bind an HTTP parameter to a Java method parameter</a>
...
Play will use the static type of the method parameter, to translate the HTTP value as a Java object.

Smart binding also works **with any classes.**
...
bc. <form action="/Directory/add" >
    Name: <input type="text" name="p.name" />
    Age: <input type="text" name="p.age" />
</form>


h2. <a>2. Redirect to an action by calling the corresponding Java method</a>
...
h2. <a>3. Don't Repeat Yourself when passing Java objects to templates</a>
...
h2. <a>4. JPA on steroids</a>

JPA is surely the best ORM API available for Java. If you know it you will be amazed how simpler it become with play. Nothing to configure, play will automatically start the JPA Entity Manager using Hibernate and magically synchronize it when code is reloaded.
...
h2. <a>5. Straightforward file upload management</a>
