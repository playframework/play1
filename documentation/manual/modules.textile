...
* A module can contains plain Java code packaged as a jar file in the module/lib directory.
...
The default modules installation path is in the **$PLAY_HOME/modules** directory. This way all modules are automatically available to all application run with the framework.
...
You can also create a standalone project that contains all modules needed by your application. You just need to copy all modules in the same directory than your application. 
...
p(note). If modules path are relative, they are resolved from the main application root.
...
h2. <a>Using the modules repository</a>

The "modules repository":http://www.playframework.org/modules identifies all modules contributed by the community. A module can have several versions. You have to check from the modules documentation which version you need to use for your framework version.

You can also browse the modules repository using the **play list-modules** command. 
...
h2. <a>Contributing a new module to the modules repository</a>
...
* Your OpenID.
* Your module must be hosted somewhere with the source code available and a way to report bugs. If you don't have any idea, github, google code or launchpad are good choices. 

To release your module, simply use the play build-module command. Then connect to the modules repository and upload the generated package.

You can of course use the offical Google Group to provide help and share about your work.
