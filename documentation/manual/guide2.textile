...
You see that the navigation between Post and Comments is not very easy: we need to use a query to retrieve all comments attached to Post. We can do better by setting up the other side of the relationship to the Post class. Add the comments field to the Post class:
...
But before continuing, it's time to save our work in bazaar. Open a command line an type **bzr st** to see the modifications made since the latest commit:
