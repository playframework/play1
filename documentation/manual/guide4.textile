...
Currently we don't validate the form content before creating the comment. We would like to set both field as required. We can easily use the play validation mechanism to ensure that HTTP parameters are correctly filled. Modify the **postComment** action to add validation annotations and check that no error occurs:
