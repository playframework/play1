...
Of course we've already written unit tests in order to test all the yabe model layer functionalities. And it's great as it will ensure that the core functionalities of the blog engine are well tested. But a web application is not only about the 'model' part. We need to ensure that the web interface works as expected. That means testing the controller layer of the yabe blog engine. But we even need to test the UI itself, as for example, our Javascript code.

h2. <a>Testing the controller part</a>

Play gives you a way to test directly the controller part of the application using JUnit. We call these tests **'Functional tests'**. This is because we want to test the complete functionality of the web application.
...
p(note). However you can still write plain HTML selenium syntax in the template and forget the specific selenium tags if needed. It can become interesting if you use one of the several selenium tools that generate the test scenarios for you, like "Selenium IDE":http://seleniumhq.org/projects/ide.
...
This test should run without any problem with the yabe application. It just opens the home page and check that the page content does not contain the 'Application error' text.

However like any complex test, you need to setup a set of well known data before navigating the application and test it. We will of course reuse the fixture concept and the **yabe/test/data.yml** file that we've used before. To import this data set before the test suite, just use the **#{fixtures /}** tag:
...
Another important thing to check is that we have a fresh user session at the test start. The session being stored in a browser transient cookie, you would keep the same session during two successive test run.
...
Of course we don't have written all required test cases for the application. But it's enough for this tutorial. Now in a real-world project, how can we know if we have written enough test cases? We need something called **'code coverage'**. 
...
As you see we're far from testing all cases of the application. A good testing suite should approach 100%, even if it is of course nearly impossible to check all the code. Typically because we often need to hack in test mode, like we did for the captcha.

p(note). Go to the "next part":guide11.