...
You can configure Play to use multiple different databases.

The default database is configured using the 'db'-key in application.conf (e.g: db.url).

If you want to configure an additional database you can do it like this:
...
This database configuration is now called 'other' in Play.

When configuring JPA for this 'other' configuration, you do it like this:

bc. db_other.jpa.dialect=<dialect>

This is how you from code access this configruation:

bc. Connection conn = DB.getDBConfig("other").getConnection()

DB.getDBConfig(configName) returns an Object with all the methods you usually find as statics on DB.
