...
Yes it works. And because the enhanced class follow Java Beans convention, when you use your object with a library which expects a JavaBean it will work perfectly !
...
1. If data is small and simple enough, store it into the flash or the session scope. However thes scopes are limited to about 4Ko each, and allow only String data.
...
Now we'll check how to perform operations outside of any HTTP request using the "jobs system":jobs.
