...
We could easily reuse the CRUD module as a start for this page but here we will create something from scratch. We need a lot of personalization on these screens.
...
We will create a form to create a new post. Basically when you have a form you have 2 actions: one to display the form and one to handle the form submission. Let's create the new **Admin.form** and **Admin.save** actions that will display and handle submission of the form:
...
We have defined the HTML form and the Java action able to create a new blog Post. But we also need to allow edition of existing posts. We can easily reuse exactly the same code with a few modifications.
...
As you can see we define this route before the old one, so it has an higher priority. That means that if an **id** parameter is submitted play will prefer this URL pattern. If not, it will still use the old route.
...
As you can see we have updated the form destination action to add the post id as first action parameter if it exists. So when the post has an id field set (meaning that the post already exists in the system), it will be sent the the **Admin.save** action. 

Now we can change the **save()** method a little to handle both creation and edition cases:
...
That's done! Now we're using the same action for both creating new blog posts and editing old ones.