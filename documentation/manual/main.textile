...
* The **Controller** processes and responds to events, typically user actions and may also invoke changes on the model. In a Web application, events are typically HTTP requests: a Controller listens for HTTP requests, extracts interesting data from the 'event', such as query string parameters, request headers... and applies changes on the underlying model objects.
...
Don't look for compiled Java classes. The play framework compiles the Java source code at runtime and only keep compiled classes to a bytecode cache in the tmp directory. The main executables artefacts in a play application are the *.java* source files, not the compiled classes.
...
You can run an application either in a DEV or PROD mode. You toggle this mode using the *application.mode* configuration property. When you run in
DEV mode, play will check for files modifications and handle hot reloading. The PROD mode is fully optimized for production mode.
