...
You can add additional configuration files to the Play configuration by specifying a file name in @application.conf@ as the value of a configuration option that has <code>@include.</code> at the start of the key. For example, if you define additional MIME types in a @conf/mime-types.conf@
