...
Create a **yabe/controllers/Security.java** file and override the **authenticate()** method:
...
Now go to "http://localhost:9000/logout":http://localhost:9000/logout to log out and you can try logging in with one of the users imported in the **initial-data.yml** file, like **bob@gmail.com/secret**.
...
But that's not yet enough to secure the CRUD section of the site! If a user knows the URL he can still access it. We have to protect controllers as well. We can do so easily using the **@Check** annotation. For example, for the **Posts** controller:
