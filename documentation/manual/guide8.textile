h1. Adding authentication and preparing the admin area
...
We've started the administration area using the CRUD module, but it is not yet well integrated with the blog UI. We will start to work for a new administration area. This one will give each author the access to his own posts. The full-featured administration using CRUD will still be available for super administrator users.
...
Now, go to the blog home page, click the 'Log in to write something' link and you should get the new administration area:

!images/guide8-2!

Good start! But as we will have several pages in this administration area we need a super template. Let's create it in the **yabe/app/views/main.html** file:
...
As you see, it is very close to the template used for the front part of the blog engine. You have replaced to **Log in** link by a **Log out** link that call the **logout** action of the **Secure** controller provided by the secure module.
...
Ok. If you try the **log out** link it will send you back to the login form:

!images/guide8-4!

That's the default way for the secure module to handle log out events. But it is easily customizable by just overriding the **onDisconnected()** method in the **controllers.Security** class:

bc. static void onDisconnected() {
    Application.index();
}

Yet, you can do the same for the **onAuthenticated()** event:
...
Well in fact we need two administration area, one for simple editors and another one for super administrator. As you seen the **User** model object has a **isAdmin** field that tell if the user has super administrator rights.

The secure module does not only provide **authentication** but **authorization** management too. It's called **profiles**. To create an **admin** profile, you just need to override the **check()** method in the **controllers.Security** class:
...
Now we can build an administration menu that depend of the user role. Update the **yabe/views/main.html** to integrate a top level menu:
...
Look have we have used the **#{secure.check /}** tag to show part of the menu only to **admin** users.

!images/guide8-5!

But it's not enough to correctly protect the CRUD part! If a user know the URL he can still access it. We have to protect controllers as well. We can do it easily using the **@Check** annotation. For example, for the **Posts** controller:
...
Do the same for the **Tags**, **Comments** and **Users** controllers. Now try to log out and log in again with a standard user (like **jeff@gmail.com/secret**). You should not see the CRUD admin links. And if you try to acces directly the **http://localhost:9000/admin/users** URL you will get a 403 response.

!images/guide8-6!

h2. <a>Customizing the CRUD layout</a>

That's fun but when we go to CRUD part of the administration area, we lost the administration layout. That is because the CRUD module provides it own layout. But we can of course override it. Use this play command:

bc. play crud:ov --layout

And you get a new **/yabe/app/views/CRUD/layout.html** template. Let's replace it content to integrate it nicely with our **admin.html** layout:
...
As you see you reuse the **crud.css** and include it in the **admin.html** using the **get/set** template variables mechanism. Now if you try the CRUD part of the administration area, it is correctly integrated with the administration layout:

!images/guide8-7!

h2. <a>Styling the login page</a>

The administration area is almost graphicaly coherent now. The last thing to style is the login page. As always it is easily customizable by overriding the default css:
...
And customize the login screen messages, by adding these lines to your own **yabe/conf/messages** file:
...
p(note). Go to the "next part":guide9. 