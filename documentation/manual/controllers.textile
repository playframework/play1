...
A date object can be automatically bound if the string representation. Using the @Bind annotation, you can specify the date format.
...
This property also affects how the dates are rendered in the templates using the ${date.format()}.
...
In order to update a list of model objects, use array notation and reference the id of the object. For example imagine the Client model has a list of Customer models declared as **List<Customer> customers**. To update the list of Customers you would provide a query string like the following:
