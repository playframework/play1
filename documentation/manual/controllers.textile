...
h3. <a name="renderjson">Return JSON String</a>

Play comes equiped with a method to simply return a JSON string, by making use of the @renderJSON(…)@ methods. These methods return a JSON string and set the content type as @application/json@. 

You can specify your own JSON string, or you can pass in an Object, which will get serialised by the @GsonBuilder@.
...
If you need more control over the JSON builder when passing in an Object to the @renderJSON(…)@ method, you can also pass in GSON serialisers and @Type@ objects to customise the output of the GSONBuilder. 

h3. <a name="renderxml">Return an XML String</a>

Similar to the JSON methods, there are also several methods for rendering XML directly from the controller. The @renderXml(…)@ methods return XML strings with the content type set as @text/xml@.

Here you can specify your own XML string, pass in a Document object, or pass in a POJO which will be serialised by the XStream serialiser.
...
Alternatively, you can use a Document object.
