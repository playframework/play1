...
Of course, cookies are signed with a secret key so the client can't modify the cookie data (or the cookie will be invalidated). The play session is not aimed to be used as a cache. If you need to cache some data related to a specific session, you can use the play built-in cache mechansim and use the *session.getId()* key to keep them related to a specific user session. 
