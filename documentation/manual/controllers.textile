...
A client uses the uniform API provided by the HTTP protocol to manipulate these resources, and, by implication, the underlying business logic. However, this mapping of resource to domain object is not a bijection : the granularity can be expressed at different levels, some resources may be virtual, some resource aliases may be defined, etc...
...
bc. public static void action(params...);
...
h2. <a name="params">Retrieve HTTP parameters</a>
...
* The request body: if the request was sent from an HTML form, the request body contains the form data encoded as **x-www-urlform-encoded**. The request body can also be a JSON or XML document.
...
If the HTTP parameter corresponding to the action method argument is not found, the corresponding method argument is set to null. If a value is found but it can't be properly cast to the required Java type, an error is added to the validation error collection and the default value is used.
...
Note that if the parameter is missing in the HTTP Request, Object type will be set to null and native types will be set to their default value.
...
The created file has the same name as the original file. It's stored in a temporary directory and deleted at the end of the request. So you have to copy it in a safe directory.
...
The binder also applies for JSON posted data. Our previous example could also be sent from an AJAX call.

bc. var cli = { name: "Zenexity", email: "contact@zenexity.fr" };
jQuery.post( "/client/create", { 'client': cli }, callback );
...
bc. public static void countUnreadMessages() {
    Integer unreadMessages = MessagesBox.countUnreadMessages();
    renderText("The are %s unread messages", unreadMessages);
}

h3. Execute a template

If the generated content is too complicated, you can use a template to generate the response content.
...
A default template is used here. The path of the default template is resolved using the Controller and action names. 
...
* The HTTP Response is **REDIRECT /clients/3132**.
...
These methods have to be **static** but not **public**. You have to annotate these method with a correction interception marker.
...
If you have to keep some data across multiple HTTP Requests, you can save it in the Session or the Flash scope. Data stored in the Session is available during the whole of the user session, and data stored in the flash scope is available to the next request only.
...
Of course, cookies are signed with a secret key so the client can't modify the cookie data (or the cookie will be invalidated).
