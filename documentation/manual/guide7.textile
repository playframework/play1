...
p(note). The convention is to pluralize the model object name to create the corresponding controller. This way, play will automatically find the associated model object for each controller. If you need to use a different name, you can still use the **@CRUD.For** annotation. Check the "manual page":crud.
...
p(note). Yes there is some groovy syntaxic sugar at work here.
