...
For now we have now way to create new blog posts are monitorate comments using the blog UI. Play provides an out of the box **CRUD** module that will help  to quickly generate a basic administration area.
...
The CRUD module is a generic application that introspects the model classes to create simple list and form.
...
bc. $ play crud:ov --template Posts/edit
...
Well this is a little hacky and we could do better here, but we have now a simpler tags selector using a little bit of javascript:

!images/guide7-6!

p(note). Go to the "next part":guide8. 