...
bc. public static void update(
		@As(lang={"fr,de","en","*"}, value={"dd/MM/yyyy","dd-MM-yyyy","MM-dd-yy"})
		Date updatedAt
	) {
	...
}
...
You can use in any action, like:
...
As you see a global binder is a classical binder annotated with *@play.data.binding.Global*. An external module can contribute binders to a project, allowing to define reusable binder extensions.
...
*host.cert* for the certificate and *host.key* for the key. If you are using keystore, then, by default it should be named *keystore.jks*.
