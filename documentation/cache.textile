...
To create highly performant systems, you sometimes need to cache data. play has a cache library intended to be used with "Memached":http://www.danga.com/memcached/.

If you don't configure memcached, play will use a simple implementation which caches data in the JVM heap.  Caching data in the JVM application breaks the "share nothing" assumption made by play : you can't start your application using several servers, and expect the application to behave consistently. Each application instance will have a different copy of the data.
