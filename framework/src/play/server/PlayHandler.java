...
public class PlayHandler extends SimpleChannelUpstreamHandler {
    ...
    protected static void addToResponse(Response response, HttpResponse nettyResponse) {
        ...
        for (Http.Cookie cookie : cookies.values()) {
            EnhancedDefaultCookie c = new EnhancedDefaultCookie(cookie.name, cookie.value);
            ...
            nettyResponse.headers().add(SET_COOKIE, PlayServerCookieEncoder.STRICT.encode(c));
        }
    ...
    private static HttpResponse addEtag(HttpRequest nettyRequest, HttpResponse httpResponse, File file) {
        ...
        boolean useWeakEtag = Play.configuration.getProperty("http.useWeakETag", "false").equals("true");
        String weakPrefix = useWeakEtag? "W/" : "";
        long last = file.lastModified();
        String etag = weakPrefix + "\"" + last + "-" + file.hashCode() + "\"";
